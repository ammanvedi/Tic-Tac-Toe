...
public class AIDrawView extends View implements OnTouchListener
{
	...
	private int touchx;
	private int touchy;
	private AIPlayer ai = new AIPlayer(3);


	public AIDrawView(Context context, int difficulty)
	{
		super(context);
		...
		if(event.getAction() == MotionEvent.ACTION_DOWN){
			...
			for(int i = 0; i < 9; i++)
			{
				if(this.testContainment(sectors[i], touchx, touchy)){
					...
					if(State.Update(new Tuple(1, i)) == false){
						System.out.println("Failed to update due to invalid move");
					}else
					{
						//succeeded
						//ai can move
						System.out.println("calling ai move");
						if(State.checkFull() == false)
						{
							State.Update(ai.aiMove(State));
						}
	...
	public void drawScore(Canvas cvs, Rect firstRect)
	{
		...
		putText(cvs, scorebox.centerX() , scorebox.centerY() + 25, "AI        : " +  Integer.toString(State.getScore()[1]));
